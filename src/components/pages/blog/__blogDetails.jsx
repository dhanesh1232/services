...
import { Calendar, Clock, Tag, Share2, Eye, ThumbsUp, MessageCircle } from "lucide-react";
...
export function BlogDetailsPage({ post }) {
  ...
  React.useEffect(() => {
    const convertBody = () => {
      ...
      container.querySelectorAll("p").forEach((p) => {
        p.className = "text-[15px] md:text-base text-foreground/70 mb-0 leading-relaxed";
      });
  ...
  const handleLike = async () => {
    try {
      ...
      toast.success(`Successfully ${data.data.liked ? "liked" : "unliked"} this post`);
    } catch (err) {
    ...
    <article className="min-h-full bg-inherit pt-20 pb-8">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 xl:px-0">
          ...
          <div className="flex flex-wrap justify-center items-center gap-4 text-muted-foreground mb-8">
            ...
            {post.readTime && (
              <div className="flex items-center gap-2 bg-card px-4 py-1 rounded-full border border-border">
                ...
                <span className="font-medium text-sm">{post.readTime + 7} min read</span>
              </div>
            )}
        ...
        {post?.featuredImage?.url && (
          <div className="relative w-full h-80 sm:h-96 mb-12 rounded-lg overflow-hidden shadow-lg border border-border">
            <Image
              src={post.featuredImage.url}
              alt={post.featuredImage.altText || post.title}
              fill
              priority
              className="object-fill transition-transform duration-300 hover:scale-[1.02]"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <aside className="lg:col-span-2 lg:sticky lg:top-16 h-fit">
            <div className="flex lg:flex-col gap-3 justify-center lg:justify-start">
              ...
              >
                ...
                <span className="hidden lg:block text-sm font-medium">{likes || 0}</span>
              </Button>
          ...
          <aside className="lg:col-span-3 hidden lg:block">
            <div className="sticky top-16 bg-card rounded-lg border border-border p-4 shadow-sm">
              <h3 className="font-semibold text-foreground mb-3 text-sm">In this article</h3>
        ...
        <footer className="mt-16 pt-8 border-t border-border">
              ...
              <div className="space-y-6">
                {post.comments.map((comment, i) => (
                  ...
                  >
                    ...
                    {/* Comment Body */}
                    <div>
                      <p className="text-foreground font-semibold">{comment.name}</p>
          ...
          <div className="bg-gradient-to-r from-primary/10 to-primary/5 rounded-xl p-6 text-center border border-primary/20">
            <h3 className="text-xl font-bold text-foreground mb-3">Enjoyed this article?</h3>
            <p className="text-muted-foreground mb-4 max-w-md mx-auto">
              Share it with your network and let me know your thoughts in the comments!
            </p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <Dialog open={showCommentForm} onOpenChange={setShowCommentForm}>
                <DialogTrigger asChild>
                  <Button variant="outline" size="sm" onClick={() => setShowCommentForm(true)}>
                    Leave a Comment
                  </Button>
                </DialogTrigger>
                <DialogContent className="bg-background">
                  <DialogHeader>
                    <DialogTitle className="text-center">Share Your Thoughts</DialogTitle>
