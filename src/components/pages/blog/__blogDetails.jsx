...
export function BlogDetailsPage({ post }) {
  ...
  return (
    <>
      ...
      <article className="min-h-full bg-background pt-20 pb-8">
            ...
            {post.tags?.length > 0 && (
              <div className="flex flex-wrap gap-2 items-center mb-8">
                ...
                {post.tags.map((tag, i) => (
                  <span
                    key={i}
                    className="text-sm font-medium bg-muted text-foreground/80 px-3 py-1.5 rounded-full border border-border hover:bg-accent transition-colors cursor-pointer"
                  >
                    #{tag}
                  </span>
                ))}
